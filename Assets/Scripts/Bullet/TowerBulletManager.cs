...
public class TowerBulletManager : Singleton<TowerBulletManager>
{
    [SerializeField]
    ...
    public void SpawnBullet(Vector3 startPos, Transform target, float damage, float speed, bool isAoe, float aoeRadius)
    {
        for(int i = 0; i < Bullets.Count; i++)
        {
            if (!BulletPool.GetChild(i).gameObject.activeInHierarchy)
            {
                Bullets[i].Init(startPos, target, damage, speed, isAoe, aoeRadius);
                break;
            }
        }
    }
